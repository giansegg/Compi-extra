"""
The Ultimate Parser App – Flask API server.

Endpoints
---------
POST /api/grammar/info          Grammar metadata (terminals, non-terminals, etc.)
POST /api/first-follow          FIRST and FOLLOW sets
POST /api/ll1/table             LL(1) prediction table
POST /api/ll1/simulate          LL(1) step-by-step simulation
POST /api/rd/simulate           Recursive Descent simulation
POST /api/lr0/table             LR(0) ACTION/GOTO tables
POST /api/lr0/simulate          LR(0) step-by-step simulation
POST /api/slr1/table            SLR(1) ACTION/GOTO tables
POST /api/slr1/simulate         SLR(1) step-by-step simulation
POST /api/lr1/table             LR(1) ACTION/GOTO tables
POST /api/lr1/simulate          LR(1) step-by-step simulation
POST /api/lalr1/table           LALR(1) ACTION/GOTO tables
POST /api/lalr1/simulate        LALR(1) step-by-step simulation
...
from lr_core import build_lr0_automaton, build_lr1_automaton, print_lr0_states, print_lr1_states
from lr_simulator import print_lr_table, simulate_lr
from lr0_parser import build_lr0_tables
from slr1_parser import build_slr1_tables
from lr1_parser import build_lr1_tables
from lalr1_parser import build_lalr1_tables
...
def ll1_table():
    ...
    # serialise table: { "NT|t": "production string" }
    table_dict = {
        f"{nt}|{t}": " ".join(prod)
...
def ll1_simulate():
    ...
    # Replay simulation manually to collect structured steps
    from grammar import EOF, EPSILON
...
def lalr1_table():
    ...
    # Count LALR states via LR(1) core merging
...
def _generate_dot_lr1(states, transitions, name: str = "LR1") -> str:
    return _generate_dot_lr0(states, transitions, name)   # items stringify the same way
...
def slr1_dot():
    ...
    if err:
        return err
    states, transitions = build_lr0_automaton(g)   # SLR(1) uses LR(0) automaton
...
def lalr1_dot():
    ...
    if err:
        return err
    states, transitions = build_lr1_automaton(g)   # LALR merges LR(1) cores
...
if __name__ == "__main__":
    print("Starting The Ultimate Parser App on http://localhost:5000")
